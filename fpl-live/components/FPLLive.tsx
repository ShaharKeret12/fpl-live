...
const DEFAULT_PLAYERS = [
  ...
  // אם תרצה דוקא את איסמעילה סאר (שלא בפרמייר ליג כיום), השאר פה כ־fallback ושנה ידנית לשחקן אחר ב־FPL
...
function PlayerPicker({ elements, value, onChange, label }:{ 
  ...
  return (
    <div className="w-full">
      ...
      <input
        className="mt-1 w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-neutral-800"
        placeholder="חפש שחקן (לדוגמה: Declan Rice)"
        value={q}
...
export default function FPLLive() {
  ...
  async function loadBootstrap() {
    try {
      ...
      setError(`שגיאה בקריאת bootstrap: ${e.message}`);
      return null;
    }
  }
  ...
  async function loadLive(eventId: number) {
    try {
      ...
      setError(`שגיאה בקריאת live: ${e.message}`);
    }
  }
  ...
  const players = useMemo(() => {
    ...
    return (selectedIds as (number|null)[])
      ...
      .map((match: any) => {
        ...
        return {
          id: match.id,
          display: `${match.first_name} ${match.second_name}`,
          team,
          cost: (match.now_cost || 0) / 10,
          seasonPoints: match.total_points,
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      {error && (
        <div className="mt-4 p-4 rounded-2xl bg-red-50 border border-red-300 text-red-800">{error}</div>
      )}
          ...
          <div key={pl.id ?? idx} className="relative bg-white rounded-2xl p-5 shadow-sm border border-neutral-200">
            <div className="flex items-center gap-4">
              <div className="w-24 h-24 rounded-xl overflow-hidden bg-neutral-100 border border-neutral-200 flex items-center justify-center">
                {pl.photoUrl ? (
                  <img src={pl.photoUrl} alt={pl.display} className="object-cover w-full h-full" />
                ) : (
            ...
            {lastUpdated && (
              <div className="text-xs opacity-60 mt-3">עודכן: {lastUpdated.toLocaleTimeString()}</div>
            )}
          </div>
        ))}
      </div>
        ...
        <div className="mt-8 rounded-2xl p-5 border border-neutral-200 bg-white shadow-sm">
          ...
          <div className="mt-2 grid grid-cols-3 items-center text-center">
            ...
            <div className="text-2xl font-bold">
              {(players[0]?.seasonPoints ?? 0) === (players[1]?.seasonPoints ?? 0)
                ? "="
                : (players[0]?.seasonPoints ?? 0) > (players[1]?.seasonPoints ?? 0)
                ? "↑"
                : "↓"}
      ...
      <footer className="mt-10 text-xs opacity-60">
        טיפ: היסטוריית מחזורים לכל שחקן דרך endpoint <code>element-summary/{`{element_id}`}</code> ומערך <code>history</code>.
      </footer>
    </div>
  );
}
